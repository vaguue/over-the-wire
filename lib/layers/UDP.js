...
// Well-known UDP port -> child layer name mapping. Either source or
// destination port matching wins; destination wins when both match (typical
...
class UDP extends UDPHeader {
  ...
  constructor(data = {}, opts = {}) {
    ...
    // UDP has a fixed 8-byte header; `length` here means header length, not the
    // datagram total (that's `totalLength`).
  ...
  nextProto(layers) {
    const Cls = layers[portProto[this.dst]] ?? layers[portProto[this.src]] ?? layers.Payload;
    // Bound the slice by the UDP datagram length so trailing Ethernet padding
    // (or any other tail bytes outside the UDP datagram) doesn't bleed into
    // the child layer's view.

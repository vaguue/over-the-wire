...
class DHCP extends DHCPHeader {
  ...
  constructor(data = {}, opts = {}) {
    if (Buffer.isBuffer(data)) {
      ...
      super(Buffer.alloc(DHCP.toAlloc(data)));
      this.merge(data);
    }
  ...
  // Writes options on top of struct-compile's scalar-field merge.
  merge(data) {
    super.merge(data);
    if (data?.options) {
  ...
  static toAlloc = (data) => baseLength + DHCP.prototype.optionsLength(data.options);

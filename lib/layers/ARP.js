...
const PCPP_ETHERTYPE_IP = 0x0800;
const PCPP_ETHERTYPE_IPX = 0x8137;
const PCPP_ETHERTYPE_IPV6 = 0x86dd;
...
class ARP extends ARPHeader {
  ...
  defaults(obj, layers) {
    ...
    if (!this.hardwareLength) {
      this.hardwareLength = 6;
    }
    if (!this.protocolType) {
      this.protocolType = PCPP_ETHERTYPE_IP;
    }
    if (!this.protocolLength) {
      this.protocolLength = 4;
    }
    if (this.opcode == 0x0001) {
      this.hardwareDst = '00:00:00:00:00:00';
    }
  }

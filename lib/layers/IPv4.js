...
class IPv4 extends IPv4Header {
  ...
  constructor(data = {}, opts = {}) {
    if (Buffer.isBuffer(data)) {
      ...
      super(Buffer.alloc(IPv4.toAlloc(data)));
      this.merge(data);
    }
  ...
  // Writes options on top of struct-compile's scalar-field merge so both the
  // standalone `new IPv4({ options: [...] })` path and the `Packet._build`
  // path (which calls `merge` after creating the layer over a sized buffer)
  // populate options the same way.
  merge(data) {
    super.merge(data);
    if (data?.options) {
   ...
   */
  get isFirstFragment() {
    return this.isFragment() && (this.fragmentOffsetValue == 0);
  }

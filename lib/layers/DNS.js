...
function parseRdata(message, type, rdataOffset, rdlength) {
  ...
  switch (type) {
    ...
    case TYPE_OPT: {
      // RFC 6891 §6.1.2 - OPT RDATA is a stream of {option-code, option-length, option-data}.
      const opts = [];
      let p = 0;
...
class DNS extends DNSHeader {
   ...
   *
   * @returns {?{udpPayloadSize: number, extendedRcode: number, version: number, doFlag: number, options: Array.<{code: number, data: Buffer}>}}
   */
  get opt() {

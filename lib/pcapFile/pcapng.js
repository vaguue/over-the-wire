...
function* readIDB(ctx, totalLength) {
  ...
  const tsresolOpt = opts.find((e) => e.option_code === constants.OPT_IF_TSRESOL);
  const nameOpt    = opts.find((e) => e.option_code === constants.OPT_IF_NAME);

  ctx.interfaces.push({
    linktype: body.linktype,
    name:     nameOpt ? nameOpt.buffer.replaceAll('\0', '') : '',
    tsresol:  tsresolOpt ? tsresolOpt.buffer : null,
  });
...
function* readEPB(ctx, totalLength) {
  ...
  const commentOpt = opts.find((e) => e.option_code === constants.OPT_COMMENT);

  const pkt = new Packet({
    iface: { linktype: iface.linktype, name: iface.name },
    buffer: buf.subarray(0, body.caplen),
    timestamp: Tsresol.parse(iface.tsresol, body),
    ...(commentOpt && { comment: commentOpt.buffer.split('\0')[0] }),
...
class PcapNGOutputStream extends Transform {
  ...
  iface({ linktype, snaplen = defaults.snaplen, tsresol = null, name = null }) {
    ...
    let tsresolBuf;
    if (tsresol == null) tsresolBuf = Buffer.from([0x09, 0x00, 0x00, 0x00]);
    else if (Buffer.isBuffer(tsresol)) tsresolBuf = tsresol;
    else tsresolBuf = Buffer.from(tsresol);
    opts.push({
    ...
    this.interfaces.push({ linktype, snaplen, name, options: opts });
  ...
  enhancedPacket(pkt) {
    ...
    const tsresolOpt = this.interfaces[interface_id].options
      .find((e) => e.option_code === constants.OPT_IF_TSRESOL);
    const tsresol = tsresolOpt ? tsresolOpt.buffer : null;
